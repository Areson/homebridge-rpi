        ...
        for (const j in config.devices) {
          ...
          switch (device.device) {
            ...
            case 'fanshim':
              break
            case 'slider':
              mandatoryKeys.push('minValue')
              parser.intKey('minValue')

              mandatoryKeys.push('maxValue')
              parser.intKey('maxValue')

              mandatoryKeys.push('minStep')
              parser.intKey('minStep')
              break
            default:
              this.warn(
          ...
          if (result.device === 'slider') {
            if (result.minValue > result.maxValue) {
              this.warn(
                'config.json: hosts[%d]: devices[%d]: %s: minValue must be less than maxValue',
                i, j, key
              )
              continue
            }
  ...
  async checkDevice (host) {
    ...
    if (host.hostname === 'localhost' || host.hostname === '127.0.0.1') {
      ...
      if (cpuInfo.model === '5' && host.devices.length > 0) {
        this.warn('%s: Raspberry Pi %s: no GPIO support', host.name, cpuInfo.model)
        host.devices = []
      }
        ...
        try {
          switch (device.device) {
            ...
            case 'valve':
              ...
              break
            case 'slider':
              await rpiAccessory.addSlider(device)
              break
          }
        } catch (error) {
