...
class RpiAccessory extends AccessoryDelegate {
  ...
  async createStatefulAccessory (device) {   
    if (RpiAccessory.StatefulAccessory == null) {
      await import('./RpiAccessory/StatefulAccessory.js')
    }
    const statefulAccessory = new RpiAccessory.StatefulAccessory(this, device)
    this.gpioAccessories[statefulAccessory.id] = statefulAccessory
    return statefulAccessory
  }

  async addSlider (device) {    
    const gpioAccessory = await this.createStatefulAccessory(device)
    if (RpiService.Slider == null) {
      await import('./RpiService/Stateful/Slider.js')
    }
    gpioAccessory.service = new RpiService.Slider(gpioAccessory, device)
    setImmediate(() => { gpioAccessory.emit('initialised') })
  }
